...
<?php
    ...
    if (isset($_GET['type'])){
        ...
        $rent = $conn->query("SELECT * FROM props WHERE type='$type'"); // selecting type from props table

        $rent->execute();

        $allsinglecatss = $rent->fetchAll(PDO::FETCH_OBJ);
    }

    if (isset($_GET['price'])){
        $price = $_GET['price'];

        $price_query = $conn->query("SELECT * FROM props ORDER BY price $price"); // selecting price (ASC/DESC) from props table

        $price_query->execute();

        $allsinglecatssPrice = $price_query->fetchAll(PDO::FETCH_OBJ);

        // var_dump($allsinglecatssPrice);//for testing temp array
    }
    ...
    if (isset($_GET['name'])){
        ...
        $singleCategory = $conn->query("SELECT * FROM props WHERE home_type= '$name'"); // selecting hometype from props table

        $singleCategory->execute();

        $allsinglecat = $singleCategory->fetchAll(PDO::FETCH_OBJ);
  ...
  <div class="slide-one-item home-slider owl-carousel">
    ...
    <div class="site-section site-section-sm bg-light">
      <div class="container">
        ...
        <div class="row mb-5">
          <?php foreach($allsinglecat as $homeType) : ?> 
          <div class="col-md-6 col-lg-4 mb-4">
            <div class="property-entry h-100">
              <a href="property-details.php?id=<?php echo $homeType->id; ?>" class="property-thumbnail">
                ...
                <img src="<?php echo THUMBNAILURL; ?>/<?php echo $homeType->image; ?>" alt="Image" class="img-fluid">
              </a>
              <div class="p-4 property-body">
                ...
                <ul class="property-specs-wrap mb-3 mb-lg-0">
                  <li>
                    ...
                    <span class="property-specs-number"><?php echo $homeType->bed; ?></span>
